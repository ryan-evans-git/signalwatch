// Package mysql provides a MySQL-backed Store implementation via the
// go-sql-driver/mysql driver. Schema and behavior mirror
// internal/store/sqlite; meaningful differences:
//   - `condition` is reserved in MySQL so it's backtick-quoted in every
//     query.
//   - Upserts use `ON DUPLICATE KEY UPDATE col=VALUES(col)` rather
//     than sqlite's `ON CONFLICT(col) DO UPDATE SET col=excluded.col`.
//   - Indexes are declared inline in CREATE TABLE rather than via
//     `CREATE INDEX IF NOT EXISTS` (which MySQL doesn't support).
package mysql
...
// Open creates a new MySQL store. dsn is forwarded to database/sql.
// A typical value is "user:pass@tcp(host:3306)/dbname?parseTime=true".
// `multiStatements=true` is required for Migrate() because the
// migration body contains multiple CREATE statements separated by `;`.
func Open(dsn string) (*Store, error) {
	if !strings.Contains(dsn, "multiStatements=") {
		...
		dsn = dsn + sep + "multiStatements=true"
	}
...
func (s *Store) Migrate(ctx context.Context) error {
	if _, err := s.db.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS schema_migrations (version VARCHAR(255) PRIMARY KEY, applied_at BIGINT NOT NULL) ENGINE=InnoDB`); err != nil {
		return err
	}
	...
	for _, name := range names {
		...
		// MySQL's go-sql-driver does not run multi-statement bodies in
		// transactions cleanly, so split on `;` boundaries. The migration
		// SQL is hand-authored so this naive split is safe.
		for _, stmt := range splitStatements(string(body)) {
			if _, err := s.db.ExecContext(ctx, stmt); err != nil {
				return fmt.Errorf("migrate %s: %w", name, err)
			}
		}
		if _, err := s.db.ExecContext(ctx, `INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)`, name, time.Now().Unix()); err != nil {
			return err
		}
	}
	return nil
...
// splitStatements naively splits a SQL script on `;` boundaries, dropping
// empty and comment-only fragments. Sufficient for the hand-written
// migrations in this package; not safe for arbitrary user-supplied SQL
// (string literals containing `;`, etc.).
func splitStatements(body string) []string {
	var out []string
	for _, raw := range strings.Split(body, ";") {
		stmt := strings.TrimSpace(raw)
		if stmt == "" || allLinesAreComments(stmt) {
			continue
		}
		out = append(out, stmt)
	}
...
func allLinesAreComments(s string) bool {
...
const ruleColumns = "id, name, description, enabled, severity, labels, input_ref, `condition`, schedule_ns, created_at, updated_at"
...
func (r *ruleRepo) List(ctx context.Context) ([]*rule.Rule, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT `+ruleColumns+` FROM rules ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *ruleRepo) ListByInput(ctx context.Context, inputRef string) ([]*rule.Rule, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT `+ruleColumns+` FROM rules WHERE input_ref = ? AND enabled = 1`, inputRef)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanRule(row rowScanner) (*rule.Rule, error) {
	var (
		id, name, severity, labelsJSON, inputRef, condJSON string
		desc                                               sql.NullString
		enabled                                            int64
...
const subscriptionColumns = "id, subscriber_id, rule_id, label_selector, dwell_ns, repeat_interval_ns, notify_on_resolve, one_shot, channel_filter, created_at, updated_at"
...
func (r *subscriptionRepo) Update(ctx context.Context, s *subscriber.Subscription) error {
	...
	_, err := r.db.ExecContext(ctx, `UPDATE subscriptions
        SET subscriber_id = ?, rule_id = ?, label_selector = ?, dwell_ns = ?, repeat_interval_ns = ?, notify_on_resolve = ?, one_shot = ?, channel_filter = ?, updated_at = ?
        WHERE id = ?`,
...
func (r *subscriptionRepo) Get(ctx context.Context, id string) (*subscriber.Subscription, error) {
	row := r.db.QueryRowContext(ctx, `SELECT `+subscriptionColumns+` FROM subscriptions WHERE id = ?`, id)
	return scanSubscription(row)
}

func (r *subscriptionRepo) List(ctx context.Context) ([]*subscriber.Subscription, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT `+subscriptionColumns+` FROM subscriptions ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *subscriptionRepo) ListForRule(ctx context.Context, ruleID string, labels map[string]string) ([]*subscriber.Subscription, error) {
	// Direct rule_id matches.
	rows, err := r.db.QueryContext(ctx, `SELECT `+subscriptionColumns+` FROM subscriptions WHERE rule_id = ?`, ruleID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	// Label-selector matches (in-memory; small subscription counts in v0.2).
	rows2, err := r.db.QueryContext(ctx, `SELECT `+subscriptionColumns+` FROM subscriptions WHERE rule_id IS NULL`)
	if err != nil {
		return nil, err
	}
	defer rows2.Close()
...
func (r *notificationRepo) ExistsForSubscription(ctx context.Context, subscriptionID string) (bool, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT 1 FROM notifications WHERE subscription_id = ? LIMIT 1`, subscriptionID,
	).Scan(&one)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return false, nil
...
// Upsert uses MySQL's `ON DUPLICATE KEY UPDATE`. We use the older
// `VALUES(col)` form rather than the new `AS new ON DUPLICATE KEY
// UPDATE col=new.col` syntax for compatibility with MySQL 8.0.0–8.0.19
// and MariaDB.
func (r *liveStateRepo) Upsert(ctx context.Context, s *rule.LiveState) error {
	...
	_, err := r.db.ExecContext(ctx, `INSERT INTO live_states (rule_id, state, triggered_at, last_eval_at, last_value, last_error, incident_id)
        ...
        ON DUPLICATE KEY UPDATE
            ...
            last_eval_at=VALUES(last_eval_at),
...
func (r *incidentSubStateRepo) Upsert(ctx context.Context, s *subscriber.IncidentSubState) error {
	...
	_, err := r.db.ExecContext(ctx, `INSERT INTO incident_sub_states (incident_id, subscription_id, last_notified_at, notify_count, resolution_sent)
        ...
        ON DUPLICATE KEY UPDATE
            ...
            resolution_sent=VALUES(resolution_sent)`,
